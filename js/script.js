...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===================================================================
// [NÂNG CẤP MỚI] HIỆU ỨNG TYPING CHO TIÊU ĐỀ TRANG
// ===================================================================
function startTitleTypingEffect() {
    // Danh sách các tiêu đề bạn muốn hiển thị
    const titles = [
        "caothong.is-a.dev"
    ];
    ...
    const typingSpeed = 150; // Tốc độ gõ chữ
    const deletingSpeed = 75;  // Tốc độ xóa chữ
    const pauseDuration = 2000; // Thời gian dừng lại sau khi gõ xong 1 tiêu đề

    function type() {
        // Lấy tiêu đề hiện tại từ mảng
        const currentTitle = titles[titleIndex];

        if (isDeleting) {
            // --- Logic XÓA CHỮ ---
            // Cắt bớt 1 ký tự khỏi tiêu đề
            ...
            if (charIndex === 0) {
                isDeleting = false;
                // Chuyển sang tiêu đề tiếp theo trong mảng
            ...
            // --- Logic GÕ CHỮ ---
            // Thêm 1 ký tự vào tiêu đề
            document.title = currentTitle.substring(0, charIndex + 1);
            charIndex++;

            // Khi đã gõ xong toàn bộ tiêu đề
            if (charIndex === currentTitle.length) {
                isDeleting = true;
                // Dừng lại một chút rồi mới bắt đầu xóa
...
// Gọi hàm này để bắt đầu hiệu ứng
